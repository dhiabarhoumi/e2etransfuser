...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=16, help='Batch size for one GPU. When training with multiple GPUs the effective batch size will be batch_size*num_gpus')
    ...
    parser.add_argument('--root_dir', type=str, default=r'/media/ros/hdd2/Porjects/e2etransfuser/transfuser_pami/data', help='Root directory of your training data')
    ...
    if not args:
        args = parser.parse_args(['--batch_size','4','--logdir', '/media/ros/hdd2/Porjects/e2etransfuser/transfuser_pami/log','--root_dir',\
            '/media/ros/hdd2/Porjects/e2etransfuser/transfuser_pami/data','--parallel_training', '0','--wandb'])   #,'--log-wandb'
    
    args.logdir = os.path.join(args.logdir, args.id)
    parallel = bool(args.parallel_training)

    if args.wandb:
        wandb.init(project=args.id, config=args,  entity="ai-mars",name = "transfuser_pmlr")
...
class Engine(object):
    ...
    def log_losses(self, loss_epoch, detailed_losses_epoch, num_batches, prefix=''):
        ...
        if (self.rank == 0):
            ...
            dic[prefix + 'loss_total'] = aggregated_total_loss
            ...
            for key, value in detailed_losses_epoch.items():
                ...
                dic[prefix + key] = aggregated_value
                dic['epoch'] = self.cur_epoch
